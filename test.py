#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#
# known attacks:
#  replay (no timestamp)
#  hmac shared secret issues
#
#  b32enc(type + id + countOrIdx [ + data]) + domain
#
import hashlib
import logging
import os
import random
import subprocess
import sys
import time
import unittest
# import pdb

import nacl.public
from mumbojumbo import (
    PacketEngine,
    Fragment,
    PublicFragment,
    DnsPublicFragment,
    encode_domain_key,
    parse_domain_key
)


# logging.basicConfig(level=logging.DEBUG)
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)


def ping_hosts(hosts):
    logger.debug('ping_hosts()')
    plist = []
    devnull = open(os.devnull, 'w')
    for host in hosts:
        cmd = 'ping -c1 -w1 {0}'.format(host)
        time.sleep(0.03)
        logger.debug('ping_hosts(): ' + cmd)
        p = subprocess.Popen(cmd.split(), stderr=devnull)
        plist.append(p)
    logger.debug('ping_hosts(): plist wait()')
    for p in plist:
        p.wait()
    devnull.close()


class DnsQueryReader(object):

    def __init__(self, iff):
        self._iff = iff

    def __iter__(self):
        cmd = 'tshark -li eth0 -T fields -e dns.qry.name udp port 53'
        self._p = p = subprocess.Popen(cmd.split(), stdout=subprocess.PIPE)
        line = p.stdout.readline().strip()
        if isinstance(line, bytes):
            line = line.decode('utf-8')
        while line:
            logger.debug('parsing ' + line)
            yield line
            logger.debug('reading next query...')
            line = p.stdout.readline().strip()
            if isinstance(line, bytes):
                line = line.decode('utf-8')
        p.wait()

    def __del__(self):
        self._p.terminate()
        self._p.wait()


def test_client(packet_engine, rounds=10):
    name_server = DnsQueryWriter()
    for i in range(rounds):
        logger.info('ROUND({0})'.format(i))
        data = nacl.public.random(random.randint(200, 400))
        logger.info('DATA({0}): {1}'.format(len(data), repr(data)))
        datahash = hashlib.sha256(data).digest()
        logger.info('HASH({0}): {1}'.format(len(datahash), repr(datahash)))
        name_server.query_all(packet_engine.to_wire(data))
        time.sleep(0.1)
        name_server.query_all(packet_engine.to_wire(datahash))
        time.sleep(0.1)
    print('SUCCESS sent {0} packets of random data plus hashes'.format(rounds))


def test_server(packet_engine, rounds=10, **kw):
    query_sniffer = DnsQueryReader(**kw)
    i = 0
    while rounds > 0:
        logger.info('ROUND({0})'.format(i))
        #
        # getting packet with random data
        #
        data = None
        datahash = None
        for dnsname in query_sniffer:
            packet_engine.from_wire(dnsname)
            if not packet_engine.packet_outqueue.empty():
                if not data:
                    data = packet_engine.packet_outqueue.get()
                    logger.info('DATA({0}): {1}'.format(len(data),
                                                        repr(data)))
                else:
                    datahash = packet_engine.packet_outqueue.get()
                    logger.info('HASH({0}): {1}'.format(len(datahash),
                                                        repr(datahash)))
                    assert datahash == hashlib.sha256(data).digest()
                    logger.info('hash OK!')
                    data = None
                    rounds -= 1
            if rounds == 0:
                break
    logger.info('SUCCESS all {0} packets had correct hashes'.format(rounds))


class DnsQueryWriter(object):

    DEFAULT_ADDR = ('127.0.0.1', 53)

    def __init__(self, name_server=DEFAULT_ADDR):
        if type(name_server) is str:
            name_server = (name_server, 53)
        self._name_server = name_server

    def _get_socket(self):
        from socket import (
            socket, AF_INET, SOCK_DGRAM
        )
        s = socket(AF_INET, SOCK_DGRAM)
        s.connect(self._name_server)
        return s

    def _build_query(self, name):
        s = b''
        s += nacl.public.random(2)  # query id
        s += b'\x01\x00'  # standard query
        s += b'\x00\x01'  # queries
        s += b'\x00\x00'  # answer rr:s
        s += b'\x00\x00'  # authority rr:s
        s += b'\x00\x00'  # additional rr:s
        for part in name.split('.'):
            s += bytes([len(part)])
            s += part.encode('ascii')
        s += b'\x00'
        s += b'\x00\x01'  # type: a, host address
        s += b'\x00\x01'  # class: in
        return s

    def query(self, name):
        s = self._get_socket()
        try:
            qry = self._build_query(name)
            s.send(qry)
        finally:
            s.close()

    def query_all(self, names):
        s = self._get_socket()
        try:
            for name in names:
                qry = self._build_query(name)
                s.send(qry)
        finally:
            s.close()


def test_dns_query():
    name_server = DnsQueryWriter()
    names = 'www.dn.se www.kernel.org whatever.asdqwe.com'.split()
    name_server.query_all(names)


def test_performance():
    _key = r'nQV+KhrNM2kbJGCrm+LlfPfiCodLV9A4Ldok4f6gvD4='
    import base64
    private_key = nacl.public.PrivateKey(base64.b64decode(_key))
    pfcls = DnsPublicFragment.bind(private_key=private_key,
                                   public_key=private_key.public_key)
    packet_engine = PacketEngine(pfcls)
    data = nacl.public.random(1024)

    count = 1024
    lst = []
    t1 = time.time()
    for i in range(count):
        for item in packet_engine.to_wire(data):
            lst.append(item)
    t2 = time.time()
    for item in lst:
        packet_engine.from_wire(item)
    t3 = time.time()

    while not packet_engine.packet_outqueue.empty():
        packet_engine.packet_outqueue.get()
        count -= 1
    assert count == 0

    print('Offline processing of 1024 messages, 1024 bytes per message:')
    print('send time: {0:.2f}s'.format(t2 - t1))
    print('recv time: {0:.2f}s'.format(t3 - t2))
    print('message fragment count:', len(lst))


class MyTestMixin(object):

    def serialize_deserialize(self, frag_cls, frag_index, frag_count,
                              frag_data):
        '''
            test deserialize(serialize()) of frag_cls
        '''
        fr1 = frag_cls(frag_index=frag_index, frag_count=frag_count,
                       frag_data=frag_data)
        fr2 = fr1.deserialize(fr1.serialize())
        assert fr1._packet_id == fr2._packet_id
        assert frag_index == fr1._frag_index == fr2._frag_index
        assert frag_count == fr1._frag_count == fr2._frag_count
        assert frag_data == fr1.frag_data == fr2.frag_data

    def multi_serialize_deserialize(self, frag_cls):
        '''
            test deserialize(serialize()) of frag_cls with:
            * zero-length data
            * one byte length data
            * 100 random data lengths between 0 and 1024
        '''
        frag_index = random.randint(0, 100)
        frag_count = random.randint(frag_index + 1, frag_index + 100)
        datalist = [b'']
        datalist += [b'a']
        datalist += [os.urandom(random.randint(0, 4096)) for i in range(100)]
        for data in datalist:
            self.serialize_deserialize(frag_cls, frag_index, frag_count, data)

    def public_serialize_deserialize(self, pfcls1, pfcls2, frag_index,
                                     frag_count, frag_data):
        '''
            test deserialize(serialize()) of frag_cls
        '''
        fr1 = pfcls1(frag_index=frag_index, frag_count=frag_count,
                     frag_data=frag_data)
        fr2 = pfcls2().deserialize(fr1.serialize())
        assert fr1._packet_id == fr2._packet_id
        assert frag_index == fr1._frag_index == fr2._frag_index
        assert frag_count == fr1._frag_count == fr2._frag_count
        assert frag_data == fr1.frag_data == fr2.frag_data

    def multi_public_serialize_deserialize(self, pfcls1, pfcls2):
        '''
            test deserialize(serialize()) of frag_cls with:
            * zero-length data
            * one byte length data
            * 100 random data lengths between 0 and 1024
        '''
        frag_index = random.randint(0, 100)
        frag_count = random.randint(frag_index + 1, frag_index + 100)
        datalist = [b'']
        datalist += [b'a']
        datalist += [nacl.public.random(random.randint(0, 4096))
                     for i in range(100)]
        for data in datalist:
            self.public_serialize_deserialize(pfcls1, pfcls2, frag_index,
                                              frag_count, data)


class Test_Fragment(unittest.TestCase):

    def test1(self):
        frag_index = 4
        frag_count = 7
        frag_data = b'foobar'
        fr1 = Fragment(frag_index=frag_index, frag_count=frag_count,
                       frag_data=frag_data)
        fr2 = fr1.deserialize(fr1.serialize())
        assert fr1._packet_id == fr2._packet_id
        assert frag_index == fr1._frag_index == fr2._frag_index
        assert frag_count == fr1._frag_count == fr2._frag_count
        assert frag_data == fr1.frag_data == fr2.frag_data


class Test_PublicFragment(unittest.TestCase, MyTestMixin):

    def do_test_cls(self, cls, **kw):
        k1 = nacl.public.PrivateKey.generate()
        k2 = nacl.public.PrivateKey.generate()
        pfcls1 = cls.bind(private_key=k1, public_key=k2.public_key, **kw)
        pfcls2 = cls.bind(private_key=k2, public_key=k1.public_key, **kw)
        self.multi_public_serialize_deserialize(pfcls1, pfcls2)

    def test_classes(self):
        self.do_test_cls(PublicFragment)
        self.do_test_cls(DnsPublicFragment, domain='.xyxyx.xy')

    def test2(self):
        self.serialize_deserialize(Fragment, frag_index=3, frag_count=4,
                                   frag_data=b'asdqwe')
        self.multi_serialize_deserialize(Fragment)


class Test_PacketEngine(unittest.TestCase, MyTestMixin):

    def setUp(self):
        packet_data_lst = [b'']
        packet_data_lst += [b'a']
        packet_data_lst += [nacl.public.random(random.randint(1, 2048))
                            for i in range(64)]
        k1 = nacl.public.PrivateKey.generate()
        k2 = nacl.public.PrivateKey.generate()
        pfcls1 = DnsPublicFragment.bind(private_key=k1,
                                        public_key=k2.public_key)
        pfcls2 = DnsPublicFragment.bind(private_key=k2,
                                        public_key=k1.public_key)
        self.packet_data_lst = packet_data_lst
        self.pfcls1 = pfcls1
        self.pfcls2 = pfcls2

    def do_test_cls(self, cls, **kw):
        pe1 = PacketEngine(frag_cls=self.pfcls1, **kw)
        pe2 = PacketEngine(frag_cls=self.pfcls2, **kw)
        for packet_data in self.packet_data_lst:
            for wire_data in pe1.to_wire(packet_data=packet_data):
                pe2.from_wire(wire_data=wire_data)
            out_data = pe2.packet_outqueue.get()
            assert packet_data == out_data
            assert pe2.packet_outqueue.empty()

    def test_classes(self):
        self.do_test_cls(PacketEngine, max_frag_data_len=100)


class Test_DomainKey(unittest.TestCase):
    """Test domain-key encoding, decoding, and integration."""

    def test_encode_decode_round_trip(self):
        """Test that encoding and decoding are inverses."""
        # Generate a random 32-byte key
        private_key = nacl.public.PrivateKey.generate()
        server_pubkey_bytes = private_key.public_key.encode()
        domain = '.example.com'

        # Encode
        domain_key = encode_domain_key(server_pubkey_bytes, domain)

        # Decode
        decoded_key, decoded_domain = parse_domain_key(domain_key)

        # Verify round-trip
        self.assertEqual(server_pubkey_bytes, decoded_key)
        self.assertEqual(domain, decoded_domain)

    def test_encode_with_leading_dot(self):
        """Test that leading dot is handled correctly in encoding."""
        private_key = nacl.public.PrivateKey.generate()
        server_pubkey_bytes = private_key.public_key.encode()

        # Test with leading dot
        domain_key1 = encode_domain_key(server_pubkey_bytes, '.example.com')
        # Test without leading dot
        domain_key2 = encode_domain_key(server_pubkey_bytes, 'example.com')

        # Both should produce the same result (no leading dot in key)
        self.assertEqual(domain_key1, domain_key2)
        self.assertNotIn('..', domain_key1)  # No double dots

    def test_decode_restores_leading_dot(self):
        """Test that parsing restores the leading dot for Mumbojumbo convention."""
        private_key = nacl.public.PrivateKey.generate()
        server_pubkey_bytes = private_key.public_key.encode()

        # Encode without leading dot
        domain_key = encode_domain_key(server_pubkey_bytes, 'example.com')

        # Decode should restore leading dot
        decoded_key, decoded_domain = parse_domain_key(domain_key)
        self.assertTrue(decoded_domain.startswith('.'))
        self.assertEqual(decoded_domain, '.example.com')

    def test_encode_invalid_key_length(self):
        """Test that encoding rejects non-32-byte keys."""
        with self.assertRaises(ValueError) as ctx:
            encode_domain_key(b'short', '.example.com')
        self.assertIn('32 bytes', str(ctx.exception))

        with self.assertRaises(ValueError) as ctx:
            encode_domain_key(b'x' * 31, '.example.com')
        self.assertIn('32 bytes', str(ctx.exception))

        with self.assertRaises(ValueError) as ctx:
            encode_domain_key(b'x' * 33, '.example.com')
        self.assertIn('32 bytes', str(ctx.exception))

    def test_parse_invalid_formats(self):
        """Test that parsing rejects invalid domain-key formats."""
        # No dot separator
        with self.assertRaises(ValueError) as ctx:
            parse_domain_key('nodothere')
        self.assertIn('at least one dot', str(ctx.exception))

        # Empty key part
        with self.assertRaises(ValueError) as ctx:
            parse_domain_key('.example.com')
        self.assertIn('empty key part', str(ctx.exception))

        # Empty domain part
        with self.assertRaises(ValueError) as ctx:
            parse_domain_key('abcd1234.')
        self.assertIn('empty domain part', str(ctx.exception))

        # Not a string
        with self.assertRaises(ValueError) as ctx:
            parse_domain_key(123)
        self.assertIn('must be a string', str(ctx.exception))

    def test_parse_invalid_base64(self):
        """Test that parsing rejects invalid base64url encoding."""
        # Invalid base64url characters (using + and / instead of - and _)
        with self.assertRaises(ValueError) as ctx:
            parse_domain_key('a+b/c==.example.com')
        self.assertIn('base64url encoding', str(ctx.exception))

    def test_parse_wrong_key_length(self):
        """Test that parsing rejects keys that aren't 32 bytes after decoding."""
        import base64
        # Encode a 16-byte key (wrong length)
        short_key = b'x' * 16
        encoded = base64.urlsafe_b64encode(short_key).decode('ascii').rstrip('=')
        domain_key = f"{encoded}.example.com"

        with self.assertRaises(ValueError) as ctx:
            parse_domain_key(domain_key)
        self.assertIn('must be 32 bytes', str(ctx.exception))

    def test_domain_key_format_length(self):
        """Test that encoded domain-key has expected length characteristics."""
        private_key = nacl.public.PrivateKey.generate()
        server_pubkey_bytes = private_key.public_key.encode()
        domain = '.example.com'

        domain_key = encode_domain_key(server_pubkey_bytes, domain)

        # Base64url encoding of 32 bytes = 43 characters (without padding)
        # Plus 1 dot + domain length
        key_part = domain_key.split('.', 1)[0]
        self.assertEqual(len(key_part), 43)

        # Verify DNS-safe characters only (base64url alphabet)
        import re
        self.assertTrue(re.match(r'^[A-Za-z0-9_-]+\.', domain_key))

    def test_from_domain_key_class_method(self):
        """Test DnsPublicFragment.from_domain_key() class method."""
        # Generate two keypairs (client and server)
        client_privkey = nacl.public.PrivateKey.generate()
        server_privkey = nacl.public.PrivateKey.generate()
        server_pubkey_bytes = server_privkey.public_key.encode()

        domain = '.test.local'

        # Create domain-key
        domain_key = encode_domain_key(server_pubkey_bytes, domain)

        # Use class method to create fragment
        frag = DnsPublicFragment.from_domain_key(domain_key, client_privkey)

        # Verify fragment is properly configured
        self.assertEqual(frag._domain, domain)
        self.assertEqual(frag._private_key, client_privkey)
        self.assertEqual(frag._public_key.encode(), server_pubkey_bytes)
        self.assertIsNotNone(frag._box)

    def test_from_domain_key_integration(self):
        """Test full integration: create fragments using domain-key and exchange data."""
        # Generate two keypairs
        k1 = nacl.public.PrivateKey.generate()
        k2 = nacl.public.PrivateKey.generate()

        domain = '.integration.test'

        # Server creates domain-key from its public key
        domain_key = encode_domain_key(k2.public_key.encode(), domain)

        # Client uses from_domain_key and then binds it to create a fragment class
        client_instance = DnsPublicFragment.from_domain_key(domain_key, k1)
        pfcls_client_bound = DnsPublicFragment.bind(
            domain=client_instance._domain,
            private_key=client_instance._private_key,
            public_key=client_instance._public_key
        )

        # Server uses traditional method
        pfcls_server_bound = DnsPublicFragment.bind(
            domain=domain,
            private_key=k2,
            public_key=k1.public_key
        )

        # Test bidirectional communication
        test_data = b'Hello from client!'

        # Client to server
        client_frag = pfcls_client_bound(
            frag_index=0,
            frag_count=1,
            frag_data=test_data
        )
        serialized = client_frag.serialize()
        server_frag = pfcls_server_bound().deserialize(serialized)
        self.assertEqual(server_frag.frag_data, test_data)

        # Server to client
        response_data = b'Hello from server!'
        server_frag = pfcls_server_bound(
            frag_index=0,
            frag_count=1,
            frag_data=response_data
        )
        serialized = server_frag.serialize()
        client_frag = pfcls_client_bound().deserialize(serialized)
        self.assertEqual(client_frag.frag_data, response_data)

    def test_domain_key_with_multi_level_domain(self):
        """Test domain-key with complex multi-level domains."""
        private_key = nacl.public.PrivateKey.generate()
        server_pubkey_bytes = private_key.public_key.encode()

        # Test various domain formats
        test_domains = [
            '.example.com',
            '.sub.example.com',
            '.deep.sub.example.com',
            '.co.uk',
            'single.tld'  # No leading dot
        ]

        for domain in test_domains:
            domain_key = encode_domain_key(server_pubkey_bytes, domain)
            decoded_key, decoded_domain = parse_domain_key(domain_key)

            self.assertEqual(server_pubkey_bytes, decoded_key)
            # Decoded domain should always have leading dot
            expected_domain = domain if domain.startswith('.') else '.' + domain
            self.assertEqual(decoded_domain, expected_domain)


def main(*args):
    import base64
    _pk1 = 'nQV+KhrNM2kbJGCrm+LlfPfiCodLV9A4Ldok4f6gvD4='
    _pk2 = 'DGor3Mkdy8Txp4bRMPYURduV7fVXcUCNnaFra1RIums='
    pk1 = nacl.public.PrivateKey(base64.b64decode(_pk1))
    pk2 = nacl.public.PrivateKey(base64.b64decode(_pk2))

    pfcls_client = DnsPublicFragment.bind(private_key=pk1,
                                          public_key=pk2.public_key)
    packet_engine_client = PacketEngine(pfcls_client)

    pfcls_server = DnsPublicFragment.bind(private_key=pk2,
                                          public_key=pk1.public_key)

    packet_engine_server = PacketEngine(pfcls_server)

    if len(args):
        if args[0] == '--test-server':
            logger.info('Now run ./mumbojumbo.py --test-client in other term')
            test_server(packet_engine_server, rounds=100, iff='eth0')
            sys.exit()

        elif args[0] == '--test-client':
            test_client(packet_engine_client, rounds=100)
            sys.exit()

        elif args[0] == '--test-dns':
            test_dns_query()
            sys.exit()

        elif args[0] == '--test-performance':
            test_performance()
            sys.exit()


if __name__ == '__main__':
    import unittest
    main(*sys.argv[1:])
    unittest.main()
