...
┌───────────────────────────────┬───────────────────────────────┤
│      Packet ID (u16)          │     Fragment Index (u16)      │
│    (Network Byte Order)       │   (Network Byte Order)        │
├───────────────────────────────┼───────────────────────────────┤
│     Fragment Count (u16)      │  Fragment Data Length (u16)   │
│   (Network Byte Order)        │   (Network Byte Order)        │
├───────────────────────────────┴───────────────────────────────┤
│                                                               │
...
Total Header Size: 8 bytes (reduced from 10 bytes)
Max Fragment Data: 80 bytes (configurable)
Max Fragment Size: 88 bytes (reduced from 90 bytes)
```
...
Fragmentation:
  ├─ Header per fragment: 8 bytes (optimized from 10 bytes)
  ├─ Fragments: ceil(N / 80)
  └─ Total fragment headers: ceil(N / 80) × 8 bytes

Encryption (per fragment):
  ├─ Nonce: 4 bytes (optimized from 24 bytes)
  ├─ Authentication tag: 16 bytes
  └─ Overhead: 20 bytes per fragment (reduced from 40 bytes)
...
Total Overhead (approximate):
  Per fragment: 8 (header) + 20 (crypto) = 28 bytes constant (44% reduction)
  ...
  For 1 KB message (1024 bytes):
    ...
    ├─ Fragment overhead: 13 × 28 = 364 bytes (was 650, saved 286 bytes)
    ├─ Base32 expansion: (1024 + 364) × 1.6 = 2221 bytes (was 2678)
    ├─ DNS overhead: 13 × 15 ≈ 195 bytes
    └─ Total transmitted: ~2416 bytes (was 2873, saved 457 bytes = 16%)

  Overhead ratio: 2416 / 1024 = 2.36× (136% overhead, improved from 180%)
```
