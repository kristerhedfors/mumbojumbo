...
┌───────────────────────────────┬───────────────────────────────┐
│      Packet ID (u16)          │                               │
│    (Network Byte Order)       │   Fragment Index (u32)        │
├───────────────────────────────┤   (Network Byte Order)        │
│                               │                               │
├───────────────────────────────┴───────────────────────────────┤
│                                                               │
...
Total Header Size: 12 bytes (u16 + u32 + u32 + u16)
Max Fragment Data: 80 bytes (configurable)
Max Fragment Size: 92 bytes
```
...
|-------|------|------|-------------|
| **Packet ID** | 2 bytes | u16 (big-endian) | Sequential identifier for the complete message (0-65535, wraps around). All fragments of the same message share this ID. Generated by incrementing counter. |
...
2. Fragment:
   ...
   ┌──────────┬──────────────┬──────────────┬──────────┬──────┐
   │  0x1234  │  0x00000000  │  0x00000001  │  0x0002  │  HI  │
   │ (pkt_id) │    (index)   │   (count)    │  (len)   │(data)│
   │   u16    │      u32     │      u32     │   u16    │ var  │
   └──────────┴──────────────┴──────────────┴──────────┴──────┘
   12 bytes header + 2 bytes data = 14 bytes plaintext

3. Encrypt with NaCl SealedBox:
   Plaintext: 14 bytes
   Ciphertext: 14 + 48 = 62 bytes (encrypted + 32-byte ephemeral pubkey + 16-byte auth tag)

4. Base32 Encode:
   62 bytes → 100 characters (base32)
   Result: "eizuwyzblbqwy3dfon2gs3thebswy3lpnyqhmzlsoqydamb3he2dkmztge3dsmrsmfzwizlsfyabcdefghijklmnop"
...
5. Parse Fragment (12-byte header):
   packet_id: 0x1234 (u16)
   frag_index: 0 (u32)
   frag_count: 1 (u32)
   frag_data_len: 2 (u16)
   frag_data: "HI"
...
Fragment 0:
  ┌──────────┬──────────────┬──────────────┬──────────┬────────────────────┐
  │ 0xDEAD   │  0x00000000  │  0x00000003  │  0x000F  │ "The quick brow"   │
  │   u16    │      u32     │      u32     │   u16    │     15 bytes       │
  └──────────┴──────────────┴──────────────┴──────────┴────────────────────┘
  Header: 12 bytes, Data: 15 bytes = 27 bytes plaintext

Fragment 1:
  ┌──────────┬──────────────┬──────────────┬──────────┬────────────────────┐
  │ 0xDEAD   │  0x00000001  │  0x00000003  │  0x000F  │ "n fox jumps ov"   │
  │   u16    │      u32     │      u32     │   u16    │     15 bytes       │
  └──────────┴──────────────┴──────────────┴──────────┴────────────────────┘
  Header: 12 bytes, Data: 15 bytes = 27 bytes plaintext
...
Fragment 2:
  ┌──────────┬──────────────┬──────────────┬──────────┬────────────────────┐
  │ 0xDEAD   │  0x00000002  │  0x00000003  │  0x000E  │ "er the lazy dog"  │
  │   u16    │      u32     │      u32     │   u16    │     14 bytes       │
  └──────────┴──────────────┴──────────────┴──────────┴────────────────────┘
  Header: 12 bytes, Data: 14 bytes = 26 bytes plaintext


Each fragment is independently:
  → Encrypted with SealedBox (adds 48 bytes overhead: ~27 + 48 = ~75 bytes)
  → Base32 encoded (~75 bytes → ~120 chars)
  → Split into DNS labels (120 chars = 2 labels of 60 chars each)
  → Domain added
...
Fragmentation:
  ├─ Header per fragment: 12 bytes (u16 + u32 + u32 + u16)
  ├─ Fragments: ceil(N / 80)
  └─ Total fragment headers: ceil(N / 80) × 12 bytes
